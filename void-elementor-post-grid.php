...
function void_grid_check_installation_time() {
    ...
    if( !$spare_me ){
        $install_info = get_option( 'void_grid_active_info' );
...
function void_grid_display_admin_notice() {
    ...
    if( $pagenow == 'index.php' ){
        $nonce = wp_create_nonce( 'vepg_spareme_nonce' );
        $dont_disturb = esc_url( get_admin_url() . '?spare_me=1&vepg_wpnonce='.$nonce );
        $plugin_info = get_plugin_data( __FILE__ , true, true );       
        $reviewurl = esc_url( 'https://wordpress.org/support/plugin/'. sanitize_title( $plugin_info['Name'] ) . '/reviews/?filter=5#new-post' );
...
function void_grid_spare_me(){ 
    ...
    if( !empty( $_GET['spare_me'] ) && !empty( $_REQUEST['vepg_wpnonce'] ) ){

        if ( !current_user_can('manage_options') || ! wp_verify_nonce($_REQUEST['vepg_wpnonce'], 'vepg_spareme_nonce') ) {
